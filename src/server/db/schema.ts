...
export const emailTemplates = pgTable("email_templates", { id: uuid("id").defaultRandom().primaryKey(), name: text("name").notNull(), category: text("category").notNull(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() });
export const emailTemplateVersions = pgTable("email_template_versions", { id: uuid("id").defaultRandom().primaryKey(), templateId: uuid("template_id").notNull().references(() => emailTemplates.id, { onDelete: "cascade" }), version: integer("version").notNull(), subject: text("subject").notNull(), body: text("body").notNull(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() }, (table) => [uniqueIndex("template_version_unique").on(table.templateId, table.version)]);
export const campaigns = pgTable("campaigns", { id: uuid("id").defaultRandom().primaryKey(), name: text("name").notNull(), objective: text("objective"), templateId: uuid("template_id").references(() => emailTemplates.id, { onDelete: "set null" }), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() });
export const campaignMembers = pgTable("campaign_members", { id: uuid("id").defaultRandom().primaryKey(), campaignId: uuid("campaign_id").notNull().references(() => campaigns.id, { onDelete: "cascade" }), directorId: uuid("director_id").references(() => directors.id, { onDelete: "set null" }), schoolId: uuid("school_id").references(() => schools.id, { onDelete: "set null" }), status: text("status").notNull().default("not_started"), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() });
export const campaignMemberStatusHistory = pgTable("campaign_member_status_history", { id: uuid("id").defaultRandom().primaryKey(), memberId: uuid("member_id").notNull().references(() => campaignMembers.id, { onDelete: "cascade" }), status: text("status").notNull(), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() });
export const draftRequests = pgTable("draft_requests", { id: uuid("id").defaultRandom().primaryKey(), campaignMemberId: uuid("campaign_member_id").references(() => campaignMembers.id, { onDelete: "set null" }), templateVersionId: uuid("template_version_id").references(() => emailTemplateVersions.id, { onDelete: "set null" }), recipient: text("recipient").notNull(), subject: text("subject").notNull(), body: text("body").notNull(), status: text("status").notNull().default("pending_review"), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() });
export const outlookDraftRefs = pgTable("outlook_draft_refs", { id: uuid("id").defaultRandom().primaryKey(), draftRequestId: uuid("draft_request_id").notNull().references(() => draftRequests.id, { onDelete: "cascade" }), externalMessageId: text("external_message_id"), composeUrl: text("compose_url"), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow() });
